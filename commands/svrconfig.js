  ...
  try {
    ...
    if (input.length === 0) {
      if (users.length === 0) {
        ...
        return 'Attempted to display registered users. Empty array found.';
      };
      const pages = Math.ceil(users.length/20);
      let userString = '';
      let limit = 20*page;
      if (users.length < limit) {
        limit = users.length;
      };
      for (let i = 20*(page - 1); i < limit; i++) {
        userString = `${userString}**${users[i].name}** (${groups[users[i].groupId].name})\n`;
      };
      ...
      if (page > pages || page < 1) {
        let pageString = '';
        if (pages === 1) {
          pageString = `There is only one page for Server #${server}.`;
        }
        else if (pages === 0) {
          await msg.channel.send(`There are no registered users on Server #${server}!`);
          return 'Attempted to display registered users. No users found in server.';
        }
        else {
          pageString = `Valid pages are between 1 and ${pages} for Server #${server}.`;
        };
        await msg.channel.send(`Invalid page. ${pageString}`);
      ...
      for (let i = 20*(page - 1); i < limit; i++) {
        userString = `${userString}**${users[i].name}** (in Group ${groups[users[i].groupId].name})\n`;
      };
      await msg.channel.send(`Registered Users on Server #${server}:\n\n${userString}\n*Page ${page}/${pages}*`);
      return 'Successfully displayed registered users in a server.';
    }
      ...
      if (/^'[^']+' /.test(input)) {
        ...
        name = input.slice(1, input.substring(1).indexOf('\'') + 1);
      }
      else if (/^"[^"]+"$/.test(input)) {
        name = input.slice(1, input.substring(1).indexOf('"') + 1);
      };
      ...
      if (page > pages || pages < 1) {
        ...
        if (pages === 1) {
          ...
          await msg.channel.send(`No users founds with \'${name}\' on Server #${server}.`);
          return 'Attempted to display registered users. No users found with given input.';
        }
        else {
          pageString = `Valid pages are between 1 and ${pages} for Server #${server}.`;
        };
      ...
      for (let i = 20*(page - 1); i < limit; i++) {
        userString = `${userString}**${results[i].name}** (in Group ${groups[results[i].groupId].name})\n`;
      };
      await msg.channel.send(`Users found with \'${name}\' on Server #${server}:\n\n${userString}`);
      return 'Successfully displayed registered users in a server.';
    }
    else {
      const results = users.filter(user => {
      ...
      if (page > pages || pages < 1) {
        ...
        if (pages === 1) {
          ...
          await msg.channel.send(`No users founds with \'${input}\' on Server #${server}.`);
          return 'Attempted to display registered users. No users found with given input.';
        }
        else {
          pageString = `Valid pages are between 1 and ${pages} for Server #${server}.`;
      ...
      await msg.channel.send(`Users found with \'${input}\' on Server #${server}:\n\n${userString}`);
      return 'Successfully displayed registered users in a server.';
    };
  }
  catch(err) {
...
async function showGroupFileInformation(msg, content) {
  try {
    ...
    if (input === '') {
      ...
      if (group.length === 0) {
        await msg.channel.send(`No groups found with the given input: \'${input}\'`);
        return 'Successfully attempted to display group info.';
      };
      let permString = '';
...
module.exports = {
  ...
  editServer: editServerConfiguration,
};